...
void write_data(uint8_t* data, uint16_t size)
{
    ...
    pkt.control_flags = (carrot_ascii_protocol_config.data_protocol.data_width << 11
        && carrot_ascii_protocol_config.data_protocol.data_encoding << 10
        && carrot_ascii_protocol_config.data_protocol.data_endian << 9);
    // TODO interleaved
    pkt.stream_id = carrot_ascii_protocol_config.data_protocol.data_channel;
    pkt.payload_len = size;
    memcpy(pkt.payload, data, sizeof(uint8_t) * size);
    memset(pkt.payload, sizeof(uint8_t) * size, sizeof(uint8_t) * (266 - 10 - size));
    pkt.crc = 0xcccc;
